...
export const FormMate = React.forwardRef<FormMateInstance, FormMateProps>((props, ref) => {
  const { renderChildren, renderItem, children, postInitialValues, grid, formMate, type, plainRender, ...rest } = props;
  ...
  const formItems = React.Children.map(children, (child, index) => {
    if (isFormItem(child) && child.props.name && (child.type as any).displayName) {
      fieldsType[child.props.name] = (child.type as any).displayName;
    }
